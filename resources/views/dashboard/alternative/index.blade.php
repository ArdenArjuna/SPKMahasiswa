          ...
          @if ($alternatives->count())
            @foreach ($alternatives as $alternative)
              <tr>
                ...
                <td class="text-center">
                  <a href="{{ url('dashboard/alternatives/'. $alternative->id) }}/edit" class="badge bg-success text-decoration-none">
                    Edit
                  </a>

                  <form action="{{ url('dashboard/alternatives/'. $alternative->id) }}" method="post" class="d-inline">
                    @method('delete')
                    @csrf
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="addAlternativeModalLabel">Add Alternatif</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <form action="{{ url('dashboard/alternatives') }} " method="post">
          <div class="modal-body">
            ...
            <ul class="list-group mb-2">
              ...
              @csrf
              <div class="my-2">
                <label for="tourism_object_id" class="form-label">Nama Mahasiswa</label>
