...
@section('content')
  ...
  <div class="table-responsive col-lg-12">
    <div class="d-lg-flex justify-content-end gap-2">
      <a href="{{ url('dashboard/criteria-comparisons') }}" class="btn btn-secondary mb-3">
        <span data-feather="arrow-left"></span>
        Back To Comparisons Data
      </a>

      @if ($isDoneCounting)
      <a href="{{ url('dashboard/criteria-comparisons/result/'. $criteria_analysis->id) }}" class="btn btn-success mb-3 ml-4">
    ...
    <table class="table table-striped">
      ...
      <tbody>
        @if (count($details))
          <form action="{{ url('dashboard/criteria-comparisons/'. $details[0]->criteria_analysis_id) }}" method="POST">
            @method('put')
            @csrf
